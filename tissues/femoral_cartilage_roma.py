...
class FemoralCartilage(Tissue):
    ...
    def unroll(self, qv_map):
        ...
        :return: tuple in (row, column) format:
                    ...
                    4. Cartilage thickness  unrolled (slices, degrees) - cartilage thickness
        """
        ...
        Cart_thick = np.float32(np.zeros((num_slices, nb_bins)))
        ...
        cart_thick_unrolled = np.transpose(Cart_thick)

        return total_cartilage_unrolled, sup_layer_unrolled, deep_layer_unrolled, cart_thick_unrolled
    ...
    def calc_quant_vals(self, quant_map, map_type):
        ...
        total, superficial, deep, thickness = self.unroll(quant_map.volume)

        assert total.shape == deep.shape
        assert deep.shape == superficial.shape
        assert superficial.shape == thickness.shape
        ...
        subject_scan = self.scan
        subject_pid = self.pid

        area_multipicator = self.__mask__.pixel_spacing[0]**2

        pd_header = ['Scan', 'Subject', 'Location', 'Side', 'Region', 'Mean', 'Std', 'Median', 'Area (mm2)', 'thickness Mean', 'Thickness Std']
        pd_list = []
        ...
        for axial in range(3):
            ...
            for coronal in coronal_direction:
                for sagittal in [self.ANTERIOR_KEY, self.CENTRAL_KEY, self.POSTERIOR_KEY]:
                    ...
                    curr_region_mask_thickness = (coronal_region_mask == coronal) * (sagital_region_mask == sagittal) * thickness

                    curr_region_mask[curr_region_mask == 0] = np.nan
                    curr_region_mask_thickness[curr_region_mask_thickness == 0] = np.nan
                    ...
                    c_area = np.count_nonzero(~np.isnan(curr_region_mask))*area_multipicator

                    if axial == 2:
                        c_thickness_mean = np.nanmean(curr_region_mask_thickness)
                        c__thickness_std = np.nanstd(curr_region_mask_thickness)
                    else:
                        c_thickness_mean = np.nanmean(curr_region_mask_thickness / 2)
                        c__thickness_std = np.nanstd(curr_region_mask_thickness / 2)


                    row_info = [subject_scan, subject_pid, axial_names[axial], coronal_names[coronal], sagittal_names[sagittal],
                                c_mean, c_std, c_median, c_area, c_thickness_mean, c__thickness_std]
